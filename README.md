...
script and you should investigate what it does, not just the obvious pools and payment addresses. I've 
commented out any settings that would obviously be unsafe on another system, such as applying overclocking, 
but the script contains most of the settings I actually use on a working rig (such as to reboot automatically 
if a GPU becomes unresponsive and open up an API to the local network), which may not be what you want in all 
cases. These are examples, not prescriptions, and should be used as such.  
...
relatively hotter. This also interacts with GPU speed and power use. For example, a rig with six Sapphire 
RX470 8GB (memory clocks 2150MHz, GPU clocks at 1155MHz) will remain stable and fairly quiet with the 
target GPU temperature at 75 degrees (producing ~26.5MH/s per card). The same rig can produce an extra 
~1Mh/s per card (for a higher power consumption) with the GPU clocks at 1265MHz. In this case, the GPU 
temperatures stay sane but targeting what would otherwise be a very reasonable GPU temperature of 70-75 
degrees allows the memory to overheat and the cards will crash because of it. Setting the target temperature 
to 60 degrees and inviting the fans to spin up to full pelt, the latter combination becomes stable enough 
for an additional 6MH/sec at the expense of some fan noise and power consumption.
...
### A bash script to start sgminer in GNU screen, with command line configuration
...
configuration. Note that this does not make any change to the API protocol, the script merely makes 
recursive calls to it covering the supplied range of values.
...
This diverges into Linux HOWTO territory as much as it says anything specific to sgminer, its API or
the bash/netcat implementation described above; the same techniques can be used with a range of JSON
data sources. But there's little point getting data to the command line unless we're able to make use
of it, so here's a few examples.
...
##### Use jq to extract the DEVS response and json2csv to present the data (-n is no header row)
...
##### And some fun with awk, just because we can.
...
Please add any bugs, todos or whatever to the issue tracker on GitHub: 

https://github.com/magick777/sgminer-recipes/issues. 

If there's anything you think I should have written or included, please have a go at implementing it and submit a pull request if you do, or describe the problem and what you're hoping to do about it. Such material as I publish here tends to be shared when others have a need for something I've already implemented or am in the throes of dealing with myself. 

## Goodwill, appreciation, beer money and so forth
